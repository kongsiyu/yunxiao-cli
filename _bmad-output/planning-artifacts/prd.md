---
stepsCompleted: [step-01-init, step-02-discovery, step-02b-vision, step-02c-executive-summary, step-03-success, step-04-journeys, step-05-domain, step-06-innovation, step-07-cli-requirements, step-08-scope, step-09-epics, step-10-architecture, step-11-final]
classification:
...
- 序列号解析（`resolveWorkitemId`）有专项测试覆盖，验证 `searchWorkitems` 全类型 + `serialNumber` 精确匹配逻辑
...
- node:test 测试（API 层全函数 + 序列号解析专项 + 命令层核心输出路径）

### Growth Features (Post-MVP)

- SKILL 文件由 skill-creator 优化生成正式版
- Shell 自动补全

### Vision (Future)

- 支持更多云效模块（流水线触发、代码审查等）
- 多组织 / 多项目快速切换
- 交互式 TUI 模式（人类友好增强）
...
- **默认**：人类可读的表格/文本输出，使用 chalk 着色
- **`--json` 模式**：完整 JSON 对象，无 chalk 着色，输出到 stdout；核心约束：**stdout 只输出纯 JSON，chalk/提示文字不得写入 stdout**（应写入 stderr），确保 LLM 可直接解析
...
| 测试 | node:test，API 层全函数 + 序列号解析专项 + 命令层核心输出路径 |
...
**兼容性：**
- Node.js ≥ 18（LTS 版本，`node:test` 原生支持）
...
- **路径前缀**：`/oapi/v1/projex/organizations/{orgId}/`
- **已知 Bug（待修复）**：listSprints 路径、searchWorkitems 默认 category、resolveWorkitemId 搜索逻辑
- **待验证**：全量 API 端点兼容性（Epic 1 Story 1.7）
...
1. 流水线 API 路径待 Epic 1 Story 1.7 全量验证后确认
2. `category: "Req,Task,Bug"` 逗号分隔在 `searchWorkitems` 中是否生效（实施中验证）
