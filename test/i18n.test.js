...
test('i18n - Language Detection', async (t) => {
  ...
  await t.test('should default to en when config language is not zh', () => {
    const lang = detectLanguage('en');
    assert.strictEqual(lang, 'en');
  });

  await t.test('should default to en when config language is undefined', () => {
    const lang = detectLanguage(undefined);
    assert.strictEqual(lang, 'en');
  });

  await t.test('should detect zh from LANG environment variable', () => {
  ...
  await t.test('should detect zh from LC_ALL environment variable', () => {
    const originalLang = process.env.LANG;
  ...
  await t.test('should default to en when locale is not zh', () => {
    const originalLang = process.env.LANG;
    const originalLC = process.env.LC_ALL;
    try {
  ...
  await t.test('config language should take precedence over locale', () => {
    const originalLang = process.env.LANG;
    try {
      process.env.LANG = 'en_US.UTF-8';
      const lang = detectLanguage('zh');
